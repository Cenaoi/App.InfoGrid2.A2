<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="TableAndStruce.ascx.cs" Inherits="App.InfoGrid2.View.OneTable.TableAndStruce" %>
...
<mi:Viewport runat="server" ID="viewport1" MarginTop="40">
    ...
    <mi:Panel runat="server" ID="Panel1" Region="West" Width="200" Scroll="None">

        <mi:Toolbar runat="server" ID="toolbar111">
            <mi:ToolBarButton Text="编辑" Command="EditTree" />
            <mi:ToolBarButton Text="刷新" Command="RefreshTree" />
        </mi:Toolbar>
        <mi:Panel runat="server" ID="leftPanel" Region="West" Width="200" Dock="Full">

            <mi:TreePanel runat="server" ID="TreePanel1" Dock="Full" OnSelected="TreePanel1_Selected">
...
<%--            <mi:ToolBarButton Text="转移" OnClick="showCataDialog(this)" />
            <mi:ToolBarHr />--%>
...
<% if (this.IsBuilder())
   { %>
<div id="SwitchPanel" style="width:500px;height:40px;border: 1px solid #C0C0C0;background-color: #FFFFFF;">
    <div style="margin:8px 24px 8px 8px; text-align:right;">

        <mi:Button ID="Button1" runat="server"  Text="工具栏定义" Command="ToolbarSetup" />
        <mi:Button runat="server" ID="Button2" Command="GoValidSetup" Text="验证设置" />

        <mi:Button ID="Button3" runat="server"  Text="列定义" Command="StepEdit2" />
        <mi:Button ID="Button4" runat="server"  Text="列设置"  Command="StepEdit3" />
        <mi:Button ID="Button5" runat="server"  Text="列高级设置"  Command="StepEdit4" />
        
        <mi:Button ID="Button6" runat="server"  Text="联动设置"  Command="GoActList" />
    </div>
</div>
...
<script type="text/javascript">
    ...
    function showCataDialog(owner) {
        var win = Mini2.create('Mini2.ui.Window', {
            ...
            url: "/App/InfoGrid2/View/Biz/Core_Catalog/TreeCataDialog.aspx?pid=<%= this.ParentCataId %>"
        });

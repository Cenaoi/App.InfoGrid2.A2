<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="StepEdit2.ascx.cs" Inherits="App.InfoGrid2.View.OneSearch.StepEdit2" %>
...
<mi:Store runat="server" ID="store2" Model="IG2_TABLE_COL" IdField="IG2_TABLE_COL_ID" PageSize="0" >
    <SelectQuery>
        <mi:QueryStringParam Name="IG2_TABLE_ID" QueryStringField="view_id" DbType="Int32" />
...
<mi:Viewport runat="server" ID="viewport">
    ...
    <mi:Panel runat="server" ID="buttonPanel" Dock="Full" Region="Center"  Scroll="None" Layout="Auto">
        ...
        <mi:Table runat="server" ID="table2"  StoreID="store2"  Dock="Full" Region="Center" Sortable="false" PagerVisible="false">
            <Columns>
                <mi:RowNumberer />
                <mi:RowCheckColumn />
                <mi:BoundField DataField="DB_FIELD" HeaderText="内部字段" />
                ...
                <mi:BoundField DataField="FILTER_TSQL_WHERE" HeaderText="T-SQL 过滤条件" Width="500" />
    ...
    <mi:WindowFooter runat="server" ID="Panel1" >
        <mi:Button runat="server" ID="OkBtn" Text="上一步" Width="80" Height="26" Command="GoPre" Dock="Center" BeforeAskText="返回上一步将清除当前设置, 确定上一步?" />

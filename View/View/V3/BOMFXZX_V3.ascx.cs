...
namespace App.InfoGrid2.View.V3
{
    public partial class BOMFXZX_V3 : WidgetControl, IView
    {
        ...
        void StoreUT101_CurrentChanged(object sender, EasyClick.Web.Mini2.ObjectEventArgs e)
        {
            ...
            try
            {
                ...
                this.StoreUT239.DataBind();
                this.StoreUT238.DataBind();
                this.StoreUT216.DataBind();
                this.StoreUT196A.DataBind();
                this.StoreUT237.DataBind();
        ...
        public void GetBomFormula()
        {
            ...
            LModel lm101 = decipher.GetModelByPk("UT_280", id101);
            ...
            //这是拿上级信息放到自身
            lmList196.ForEach((lm) =>{  GetParentInfo(lm, lm102, lmList196); });
        ...
        public void ClearBOM()
        {
            ...
            try
            {
                ...
                foreach (var lm102 in lm102List)
                {
                    ...
                    LightModelFilter lmFilter238 = new LightModelFilter("UT_238");
                    lmFilter238.And("ROW_SID", 0, HWQ.Entity.Filter.Logic.GreaterThanOrEqual);
                    lmFilter238.And("UT_102_ID", lm102["ROW_IDENTITY_ID"]);
                    lmFilter238.And("COL_74", id101);
                    //删除
                    decipher.UpdateProps(lmFilter238, new object[] { "ROW_SID", -3, "ROW_DATE_DELETE", DateTime.Now });

                    LightModelFilter lmFilter239 = new LightModelFilter("UT_239");
                    lmFilter239.And("ROW_SID", 0, HWQ.Entity.Filter.Logic.GreaterThanOrEqual);
                    lmFilter239.And("UT_102_ID", lm102["ROW_IDENTITY_ID"]);
                    lmFilter239.And("COL_74", id101);
                    //删除
                    decipher.UpdateProps(lmFilter239, new object[] { "ROW_SID", -3, "ROW_DATE_DELETE", DateTime.Now });
        ...
        public void ClearBOMAll()
        {
            ...
            try
            {
                ...
                LightModelFilter lmFilter102 = new LightModelFilter("UT_281");
                lmFilter102.And("ROW_SID", 0, HWQ.Entity.Filter.Logic.GreaterThanOrEqual);
                ...
                LightModelFilter lmFilter238 = new LightModelFilter("UT_238");
                lmFilter238.And("ROW_SID", 0, Logic.GreaterThanOrEqual);
                lmFilter238.And("COL_74", id101);
                //删除
                decipher.UpdateProps(lmFilter238, delParam);

                LightModelFilter lmFilter239 = new LightModelFilter("UT_239");
                lmFilter239.And("ROW_SID", 0, Logic.GreaterThanOrEqual);
                lmFilter239.And("COL_74", id101);
                //删除
                decipher.UpdateProps(lmFilter239, delParam);
                ...
                LightModelFilter lmFilter237 = new LightModelFilter("UT_237");
                lmFilter237.And("ROW_SID", 0, Logic.GreaterThanOrEqual);
                lmFilter237.And("COL_74", id101);
                //删除
                decipher.UpdateProps(lmFilter237, delParam);
                ...
                this.StoreUT239.Refresh();
                this.StoreUT238.Refresh();
                this.StoreUT196A.Refresh();
                this.StoreUT237.Refresh();
        ...
        public void NumFormula()
        {
            ...
            //	UT_102	吸塑指令单-订单明细
            LModel lm102 = decipher.GetModelByPk("UT_281", id102);
        ...
        /// <summary>
        /// 从UT_196数据表中拿数据到UT_238和UT_239和UT_216
        /// </summary>
        public void GetUT196To238()
        {
            int id = StringUtil.ToInt(this.StoreUT101.CurDataId);

            DbDecipher decipher = ModelAction.OpenDecipher();

            LModel lm101 = decipher.GetModelByPk("UT_280", id);
            ...
            LightModelFilter lmFilter239 = new LightModelFilter("UT_196");
            lmFilter239.And("COL_20", id);
            lmFilter239.And("ROW_SID", 0, Logic.GreaterThanOrEqual);
            lmFilter239.And("COL_13", "外发加工");

            //这是拿到要放到UT_239 委外加工
            List<LModel> lm239List = decipher.GetModelList(lmFilter239);
            ...
            List<LModel> lm238List = new List<LModel>();
            List<LModel> lm237List = new List<LModel>();



            //这是UT_239 数据集合
            List<LModel> lm239New = new List<LModel>();

            //这是UT_216 数据集合
            List<LModel> lm216New = new List<LModel>();

            //这是UT_238 数据集合
            List<LModel> lm238New = new List<LModel>();

            //这是UT_237 数据集合
            List<LModel> lm237New = new List<LModel>();



            //循环映射每一条数据 239
            foreach (var item in lm239List)
            {
                LModel lm239 = new LModel("UT_239");

                MapMgr.MapData(67, item, lm239);


                LightModelFilter lmFilter237 = new LightModelFilter("UT_196");
                lmFilter237.And("BIZ_PARENT_ID", item.GetPk());
                lmFilter237.And("ROW_SID", 0, Logic.GreaterThanOrEqual);
                //拿到下级数据
                var lms = decipher.GetModelList(lmFilter237);

                lm237List.AddRange(lms);
            ...
            foreach (var item in lm238List)
            {

                LModel lm238 = new LModel("UT_238");


                MapMgr.MapData(66, item, lm238);
            ...
            //给每一条记录都加上 UT_101 的 ID
            foreach (var item in lm237List)
            {
                LModel lm237 = new LModel("UT_237");


                MapMgr.MapData(64, item, lm237);
            ...
            LD(lm239New, this.StoreUT239, decipher, dbCascadeFactory);

            LD(lm216New, this.StoreUT216, decipher, dbCascadeFactory);

            LD(lm238New, this.StoreUT238, decipher, dbCascadeFactory);

            LD(lm237New, this.StoreUT237, decipher, dbCascadeFactory);
        ...
        /// <summary>
        /// 从UT_103数据表中拿数据放到 196 238 239 216 表中
        /// </summary>
        public void GetUT103Bom()
        {
            ...
            //这是UT_239表的集合
            List<LModel> lm239List = new List<LModel>();
            ...
            foreach (var item in lm103List)
            {
                ...
                //从UT_103 映射到 UT_238
                MapMgr.MapData(67, item, lm238);

                //从UT_103 映射到 UT_239
                MapMgr.MapData(66, item, lm239);
            ...
            decipher.InsertModels<LModel>(lm196List);
            decipher.InsertModels<LModel>(lm238List);
            decipher.InsertModels<LModel>(lm239List);
            decipher.InsertModels<LModel>(lm216List);
        ...
        public void GetUT219To196()
        {
            ...
            //
            int ut_280_id = int.Parse(StoreUT101.CurDataId);
            ...
            LModel lm101 = decipher.GetModelByPk("UT_280", ut_280_id);
        ...
        /// <summary>
        /// 确认分析结果按钮事件 把UT_238、UT_239、UT_216 的 BIZ_SID 字段从 0 改为 2
        /// </summary>
        public void BizSid0_2()
        {
            ...
            //UT_238 表 集合
            filter = new FilterForm("UT_238").Where("ROW_SID >=0 and COL_74={0} and BIZ_SID = 0", id101).Filter;
            List<LModel> lm238List = decipher.GetModelList(filter);

            //UT_239 表 集合
            filter = new FilterForm("UT_239").Where("ROW_SID >=0 and COL_74={0} and BIZ_SID = 0", id101).Filter;
            List<LModel> lm239List = decipher.GetModelList(filter);
            ...
            //UT_237 表 集合
            filter = new FilterForm("UT_237").Where("ROW_SID >=0 and COL_74={0} and BIZ_SID = 0", id101).Filter;
            List<LModel> lm237List = decipher.GetModelList(filter);
            ...
            if (lm216List.Count == 0 && lm239List.Count == 0 && lm238List.Count == 0 && lm196List.Count == 0 && lm237List.Count == 0)
            {
            ...
            try
            {
                ...
                lm238List.ForEach((lm) => { UpdataBizSid(lm, 2, decipher, dbCascadeFactory, "UT_238"); });
                lm239List.ForEach((lm) => { UpdataBizSid(lm, 2, decipher, dbCascadeFactory, "UT_239"); });
                lm196List.ForEach((lm) => { UpdataBizSid(lm, 2, decipher, dbCascadeFactory, "UT_196"); });
                lm237List.ForEach((lm) => { UpdataBizSid(lm, 2, decipher, dbCascadeFactory, "UT_237"); });
        ...
        /// <summary>
        /// 撤销分析结果按钮事件 把UT_238、UT_239、UT_216 的 BIZ_SID 字段从 2 改为 0
        /// </summary>
        public void BizSid2_0()
        {
            ...
            //UT_238 表 集合
            filter = new FilterForm("UT_238").Where("ROW_SID >=0 and COL_74={0} and BIZ_SID = 2", id101).Filter;
            List<LModel> lm238List = decipher.GetModelList(filter);

            //UT_239 表 集合
            filter = new FilterForm("UT_239").Where("ROW_SID >=0 and COL_74={0} and BIZ_SID = 2", id101).Filter;
            List<LModel> lm239List = decipher.GetModelList(filter);
            ...
            //UT_237 表 集合
            filter = new FilterForm("UT_237").Where("ROW_SID >=0 and COL_74={0} and BIZ_SID = 2", id101).Filter;
            List<LModel> lm237List = decipher.GetModelList(filter);
            ...
            try
            {
                ...
                lm238List.ForEach((lm) => { UpdataBizSid(lm, 0, decipher, dbCascadeFactory, "UT_238"); });
                lm239List.ForEach((lm) => { UpdataBizSid(lm, 0, decipher, dbCascadeFactory, "UT_239"); });
                lm196List.ForEach((lm) => { UpdataBizSid(lm, 0, decipher, dbCascadeFactory, "UT_196"); });
                lm237List.ForEach((lm) => { UpdataBizSid(lm, 0, decipher, dbCascadeFactory, "UT_237"); });

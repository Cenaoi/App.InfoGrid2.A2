...
namespace App.InfoGrid2.View.CustomPage
{
    ...
    /// 应收应付管理/收款单
    /// </summary>
    public class T302_145_Page: ExPage
    {
        ...
        protected override void OnLoad()
        {
            ...
            if (!IsPostBack)
            {
                ...
                Toolbar tbar = this.FindControl("toolbarT2564") as Toolbar;

                ToolBarButton btn = new ToolBarButton("勾兑应收款单据");
                btn.OnClick = GetMethodJs("DaoRu", "你好");
                tbar.Items.Add(btn);

                ToolBarButton atuoMoney = new ToolBarButton("自动收款");
        ...
        public void DaoRu(string name)
        {
            ...
            string filter2 = Base64Util.ToString("[{field:'UT_185.COL_15',logic:'=',value:'" + COL_12 + "'}]", Base64Mode.Http);
            
            win.ContentPath = string.Format("/App/InfoGrid2/View/MoreView/DataImportDialog.aspx?id=683&filter2={0}", filter2);
        ...
        public void AutoReceivables(string moneyStr)
        {
            ...
            lmFilter.And("COL_1",id125);
            lmFilter.And("ROW_SID",0, HWQ.Entity.Filter.Logic.GreaterThanOrEqual);
            lmFilter.Fields = new string[] { "ROW_IDENTITY_ID", "COL_21", "COL_24" };
            ...
            lm126List.ForEach(m => num += m.Get<decimal>("COL_21"));

            if (money > num)
            {

                string popJs = "Mini2.Msg.confirm('提示','" + string.Format("现收金额大于应收款！点击确定，将核销{0}元。多出{1}元可在预收款单手工录入！", num, money - num) + "'," +
       ...
       public void AoutOk(decimal money)
        {
            ...
            try
            {
                ...
                {
                    if (money - lm.Get<decimal>("COL_21") < 0)
                    {

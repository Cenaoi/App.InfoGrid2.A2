...
{
    public partial class CataTree : WidgetControl, IView
    {
        ...
        void TreePanel1_NodeMoved(object sender, TreeNodeMoveEventArgs e)
        {
            ...
            BIZ_CATALOG curModel = decipher.SelectModelByPk<BIZ_CATALOG>(id);

            LModelList<BIZ_CATALOG> models;

            if (curModel.PARENT_ID == pId)
            {
                models = decipher.SelectModels<BIZ_CATALOG>(LOrder.By("SEQ,BIZ_CATALOG_ID"), "PARENT_ID = {0} AND BIZ_CATALOG_ID <> {1}", pId, id);
            }
            else
            {
                models = decipher.SelectModels<BIZ_CATALOG>(LOrder.By("SEQ,BIZ_CATALOG_ID"), "PARENT_ID = {0}", pId);
            }

            models.Insert(e.Position, curModel);

            int i = 0;



            foreach (BIZ_CATALOG item in models)
            {
                item.SEQ = i++;
        ...
        void TreePanel1_Creating(object sender, EventArgs e)
        {
            ...
            BIZ_CATALOG pCata = decipher.SelectModelByPk<BIZ_CATALOG>(parent.Value);
        ...
        /// 创建默认根节点
        /// </summary>
        private BIZ_CATALOG CreateDefaultRoot(BIZ_CATALOG rootCata, int rootId)
        {
            if (rootCata == null)
            {
                rootCata = new BIZ_CATALOG();
                rootCata.CATA_TEXT = "根分类";
                rootCata.CATA_CODE = "ROOT";
                rootCata.CATA_TYPE_CODE = "DIR";
                rootCata.BIZ_CATALOG_ID = rootId;


                DbDecipher decipher = ModelAction.OpenDecipher();

                decipher.IdentityStop();
                decipher.InsertModel(rootCata);
        ...
        public void InitData()
        {
            ...
            BIZ_CATALOG rootCata = decipher.SelectModelByPk<BIZ_CATALOG>(rootId);

            rootCata = CreateDefaultRoot(rootCata, rootId);

            TreeNode rNode = new TreeNode(rootCata.CATA_TEXT, rootCata.BIZ_CATALOG_ID);
            rNode.ParentId = rootCata.PARENT_ID.ToString();
            rNode.NodeType = "ROOT";
            rNode.StatusID = 0;
            rNode.Expand();
        ...
        protected void TreePanel1_Selected(object sender, EventArgs e)
        {
            ...
            MiniPager.Redirect("iform1",
                 string.Format("/App/InfoGrid2/View/Biz/Core_Catalog/CataEdit.aspx?id={0}", id));

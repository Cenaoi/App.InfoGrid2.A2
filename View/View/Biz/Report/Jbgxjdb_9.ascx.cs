...
namespace App.InfoGrid2.View.Biz.Report
{
    ...
    /// 小渔夫写的
    /// </summary>
    public partial class Jbgxjdb_9 : WidgetControl, IView
    {
        ...
        void ConstructionJavascript(RTable table)
        {
            ...
            string m_dateList = string.Empty;  // CreateDateList(table);

            CrossHeadTreeNode chrn = cr.RowTreeRoot;


            //这是统计那一行对象
            CrossRowHeadTreeNode crht = null;


            //定义几条线
            Dictionary<int,string> legend = new Dictionary<int,string>();

            int j = cr.RowGroupTags.Count;
            ...
            foreach (var item in cr.HeadTreeRoot.Childs)
            {
                ...
                foreach(var sub_item in item.Childs)
                {
                    legend.Add(j++, sub_item.Column.Text);
                }
            ...
            foreach(var item in table.Body)
            {
                ...
                var item0 = item[0];
                //这是把总计给去除掉
                if(item0.TreeNode.NodeType == CrossHeadTreeNodeTypes.Total)
                {
                    continue;
                }

                xAxis.Add(item0.Value.ToString());

            }

            SModelList new_sms = new SModelList();
            ...
            foreach (var item in legend)
            {
                ...
                foreach (var body in table.Body)
                {
                    ...
                    //这是把总计给去除掉
                    if (itemFirt.TreeNode.NodeType == CrossHeadTreeNodeTypes.Total)
                    {
                        continue;
                    }


                    var item0 = body[item.Key];

                 
                  
                    values.Add(Convert.ToDecimal(item0.Value));
            ...
            StringBuilder legendJson = new StringBuilder();
            ...
            foreach (var item in legend)
            {
                ...
                if(i++> 0) { legendJson.Append(","); };

                legendJson.Append($"'{item.Value}'");
            }


            legendJson.Append("]");

            StringBuilder xAxisJson = new StringBuilder();
            ...
            {
                if (i++ > 0) { xAxisJson.Append(","); };


                xAxisJson.Append($"'{item}'");
            }

            xAxisJson.Append("]");


            ScriptManager.Eval("setChart(" + xAxisJson + ","+ legendJson + "," + new_sms.ToJson() + ");");

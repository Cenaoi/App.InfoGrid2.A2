...
{
    public partial class Jbgxjdb_1 : WidgetControl, IView
    {
        ...
        private void CreateHeader(CrossHeadTreeNode parentNode, TableColumnCollection cols)
        {
            ...
            foreach (var node in parentNode.Childs)
            {
                ...
                if(node.NodeType == CrossHeadTreeNodeTypes.Total)
                {
                    continue;
                }
                ...
                if (node.HasChild())
                {
                    ...
                    key_value.Add(node.Column.Text, cols_1);
        ...
        Dictionary<string, List<int>> key_value = new Dictionary<string, List<int>>();
        ...
        void ConstructionJavascript(RTable table)
        {
            ...
            foreach (CrossHeadTreeNode item in chrn.Childs)
            {
                ...
                if (item.NodeType == CrossHeadTreeNodeTypes.Total)
                {
                     ...
                     crht = item as CrossRowHeadTreeNode;
            ...
            Dictionary<string, SModelList> dic = new Dictionary<string, SModelList>();
            ...
            foreach (var item in key_value)
            {
                ...
                foreach (int sub_item in item.Value)
                {
                    ...
                    RCell rc = crht.Data.Find(d => d.X == sub_item);
                    ...
                    string key = rc.TreeNode.Column.Text;


                    if (!dic.ContainsKey(key))
                    {

                        SModelList sms_1 = new SModelList();
            ...
            ScriptManager.Eval("setChart(" + m_dateList + "," + m_titleList + "," + new_sms.ToJson() + ");");

...
{
    public partial class Jbgxjdb_8 : WidgetControl, IView
    {
        ...
        protected override void OnInitCustomControls(EventArgs e)
        {
            ...
            this.headLab.Value = "<span class='page-head' >" + "生产管理报表" + "</span>";
        ...
        public string chart_data5 = string.Empty;


        public void GetChartData5()
        {
            DbDecipher decipher = ModelAction.OpenDecipher();

            string sql = "select max(ROW_DATE_CREATE) as MAXDATE  from UT_317 where ROW_SID>=0 ";
            ...
            //LightModelFilter filter = new LightModelFilter("UT_317");
            //filter.And("ROW_SID", 0, Logic.GreaterThanOrEqual);
            //filter.And("COL_113","JS16009");
            ////filter.And("COL_16", "原材料", Logic.Inequality);
            //filter.And("ROW_DATE_CREATE", maxdate, Logic.GreaterThanOrEqual);

            string sql1 = $"select SUM(COL_76)/SUM(COL_83)*100 as QUALIFIED,COL_117  from  UT_317   where ROW_SID>=0 and ROW_IDENTITY_ID in ";

            sql1 += $" (select ROW_IDENTITY_ID from UT_317  where ROW_DATE_CREATE >= '{begTime}' and ROW_DATE_CREATE <= '{endTime}' and ROW_SID>=0 and COL_113='JS16009' and COL_16<>'原材料' ) group by COL_117 ";

            SModelList sms = decipher.GetSModelList(sql1);
            ...
            {
                if (string.IsNullOrWhiteSpace(item["COL_117"]))
                {
                    continue;
                }

                title.Add($"{item.Get<string>("COL_117")}");

                decimal qualified = decimal.Round(item.Get<decimal>("QUALIFIED"), 2);

                data1.Add($"{qualified.ToString()}");
            ...
            sm["title"] = title.ToArray();
            sm["data1"] = data1.ToArray();

            chart_data5 = sm.ToJson();

        }


        public string chart_data6 = string.Empty;


        public void GetChartData6()
        {
            DbDecipher decipher = ModelAction.OpenDecipher();
            ...
            string sql1 = $"select SUM(COL_76)/SUM(COL_83)*100 as QUALIFIED,COL_4,COL_5  from  UT_317   where ROW_SID>=0 and ROW_IDENTITY_ID in ";

            sql1 += $" (select ROW_IDENTITY_ID from UT_317  where ROW_DATE_CREATE >= '{begTime}' and ROW_DATE_CREATE <= '{endTime}'  and ROW_SID>=0 and COL_113='JS16009' and COL_16<>'原材料') group by COL_4,COL_5 ";
            ...
            foreach (var item in sms)
            {
                if (string.IsNullOrWhiteSpace(item["COL_4"]))
                {
                    continue;
                }

                title.Add($"{item.Get<string>("COL_4") + "-" + item.Get<string>("COL_5")}");

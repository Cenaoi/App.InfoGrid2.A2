...
namespace App.InfoGrid2.View.Biz.JLSLBZ.Currency
{
    ...
    /// </summary>
    public partial class FormXssczlV2 : WidgetControl, IView
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            DbCascadeRule.Bind(StoreUT103);
            DbCascadeRule.Bind(StoreUT104);
            DbCascadeRule.Bind(StoreUT102);


            if (!this.IsPostBack)
            {

                this.headLab.Value = "<span class='page-head' >车间生产指令单</span>";
        ...
        private void InitData()
        {
            ...
            this.comboBox5.Value = IfUtil.NotBlank(model["COL_31"], "101");
        ...
        public void GoSubmit()
        {
            ...
            model["COL_30"] = this.comboBox5.Value == "101" ? "定制件" : "量产件";
            model["COL_31"] = this.comboBox5.Value;
            model["BIZ_SID"] = 2;
        ...
        /// 引入生产计划弹出窗口确定按钮事件
        /// </summary>
        /// <param name="ids_str">选中的UT_281的id字符串</param>
        public void GoImportProductionPlan(string ids_str)
        {

            int[] ids = StringUtil.ToIntList(ids_str);
            ...
            int id_101 = WebUtil.QueryInt("id");


            DbDecipher decipher = ModelAction.OpenDecipher();

            List<LModel> lm102s = new List<LModel>();
            ...
            try
            {
                ...
                {
                    LModel lm281 = decipher.GetModelByPk("UT_281", id);

                    LModel lm102 = new LModel("UT_102");

                    MapMgr.MapData(92, lm281, lm102);

                    lm102["COL_13"] = id_101;
                    lm102["ROW_DATE_CREATE"] = lm102["ROW_DATE_UPDATE"] = DateTime.Now;
                ...
                decipher.InsertModels(lm102s);

                StoreUT102.Refresh();

                Toast.Show("引入生产计划成功了！");
        ...
        /// </summary>
        /// <param name="ids_str">选中的UT_281的id字符串</param>
        public void GoImportProductionPlan_107(string ids_str)
        {
            ...
            try
            {
                ...
                foreach (int id in ids)
                {
                    LModel lm370 = decipher.GetModelByPk("UT_370", id);

                    LModel lm102 = new LModel("UT_102");

                    MapMgr.MapData(107, lm370, lm102);
        ...
        public void GoAutoInputData()
        {
            ...
            LModel lm101 = decipher.GetModelByPk("UT_101", id_101);

            if(lm101.Get<int>("COL_15") == 2)
            {
                Toast.Show("已经分析过了！");
                return;
            }
            ...
            try
            {
                ...
                EasyClick.Web.Mini.MiniHelper.EvalFormat("ShowGif('智能分析成功了！')");
        ...
        public void GoGetStandardBOM()
        {
            ...
            if (lm101 == null)
            {
                log.Error($"获取不到数据!UT_101的ID:{id_101}");
                MessageBox.Alert("获取不到数据!");
                return;
            }

            if (lm101.Get<int>("BIZ_SID") > 0)
            {
                MessageBox.Alert("单据已审核，无法执行！");
                return;
            }

            LightModelFilter filter = new LightModelFilter("UT_102");
            filter.And("ROW_SID", 0, Logic.GreaterThanOrEqual);
            filter.And("COL_13", id_101);

            LModelList<LModel> lms102 = decipher.GetModelList(filter);

            lms102.BatchSetTakeChange(true);

            lms102.BatchSet("COL_57", DateTime.Now);
            lms102.BatchSet("COL_62", 3);
            lms102.BatchSet("COL_84", 2);


            try
            {
                DbCascadeRule.Update(lms102);

                EasyClick.Web.Mini.MiniHelper.EvalFormat("ShowGif('获取标准BOM成功了！')");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Alert("哦噢，获取标准BOM出错了！");
        ...
        /// 撤销工艺分析 按钮点击事件
        /// </summary>
        public void GoChangeCOL_15_2To1()
        {
        ...
        /// 显示单表打印选择界面
        /// </summary>
        public void GoShowSelectPrintView()
        {
            ...
            //主表表名
            string mainTable = "UT_101";

            //主表主键字段
            string mainPK = "ROW_IDENTITY_ID";

            //主表当前记录ID
            int mainTableID = WebUtil.QueryInt("id");
            ...
            NameValueCollection nv = new NameValueCollection(){
                {"id",page_id.ToString()},
                {"mainTableID",mainTableID.ToString()},  //主数据记录的ID
        ...
        public void Win_WindowClosed(object sender, string data)
        {
            ...
            Excel_Template.V1.MoreSubTableDataSet ds = new Excel_Template.V1.MoreSubTableDataSet();
            ...
            try
            {
                ...
                Excel_Template.V1.SheetParam sp = Excel_Template.V1.TemplateUtilV1.ReadTemp(path);

                Excel_Template.V1.TemplateUtilV1.CreateExcel(sp, ds, wFile.PhysicalPath);

                pathUrl = wFile.RelativePath;
        ...
        /// </summary>
        /// <returns></returns>
        Excel_Template.V1.MoreSubTableDataSet GetDataSet()
        {

            Excel_Template.V1.MoreSubTableDataSet ds = new Excel_Template.V1.MoreSubTableDataSet();

            int ut_101_id = WebUtil.QueryInt("id");


            DbDecipher decipher = ModelAction.OpenDecipher();

           
            SortedList<string, LModelList<LModel>> items = new SortedList<string, LModelList<LModel>>();


            LModel lm101 = decipher.GetModelByPk("UT_101", ut_101_id);


            LModelList<LModel> lm102s = StoreUT102.GetList() as LModelList<LModel>;


            LModelList<LModel> lm103s = StoreUT103.GetList() as LModelList<LModel>;

            LModelList<LModel> lm313s = StoreUT313.GetList() as LModelList<LModel>;

            ds.Head = lm101;

            items.Add(StoreUT102.Model,lm102s);
            items.Add(StoreUT103.Model,lm103s);
            items.Add(StoreUT313.Model,lm313s);
        ...
        /// <summary>
        /// 引入项目定制订单、引入项目量产订单弹出窗口确定按钮事件
        /// </summary>
        public void GoImportProductionPlan_115(string ids_str)
        {
            int[] ids = StringUtil.ToIntList(ids_str);
            ...
            try
            {
                MapSet mapSet = MapSet.SelectSID_0(decipher, 115);

                foreach (int id in ids)
                {
                    LModel lm091 = decipher.GetModelByPk(mapSet.Map.R_TABLE, id);

                    LModel lm102 = new LModel(mapSet.Map.L_TABLE);

                    MapMgr.MapData(115, lm091, lm102);
        ...
        public void GoExport()
        {
            ...
            win.WindowClosed += Win_WindowClosedV1;
        ...
        /// <param name="sender"></param>
        /// <param name="data">数据</param>
        public void Win_WindowClosedV1(object sender, string data)
        {
            ...
            try
            {
                ...
                string dir = "/_Temporary/Excel/";
                string f_path = dir + fileName + ".xls";

                WebFileInfo wFile = new WebFileInfo(dir, fileName + ".xls");
                wFile.CreateDir();
                ...
                string downloa = "Mini2.create('Mini2.ui.extend.DownloadWindow', {fileName: '下载 Excel 文件',fielUrl:'" + f_path + "'}).show();";

...
function CleverTabs(self, options) {
    ...
    this.options = $.extend({
        ...
        //Tab用于控制的头模板
        tabHeaderTemplate: '<li id="cleverTabHeaderItem-#{id}" class="#{liclass}" style="padding:0 16px 0 0;"><a href="#" title="#{title}">#{label}</a></li>',
        //Tab用于显示的Panel模板
    ...
    self.addClass("ui-tabs ui-widget");
    ...
    this.panelContainer.addClass("ui-corner-bottom");
...
CleverTabs.prototype.getTabByUrl = function (url) {
    ...
    for (i = 0; i < frames.size(); i++) {
        ...
        var ff = frame.get(0);
        var src = ff.contentWindow.location.href;

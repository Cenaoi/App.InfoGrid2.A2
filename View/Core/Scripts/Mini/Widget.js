...
Mini.ui.Widget = function (ps) {
    ...
    function ProExData(exData, exPath) {
        ...
        if (exData.callback_owner) {
            exPath.__callback_owner = exData.callback_owner;
        }

        if (exData.rdata) {
            exPath.__rdata = exData.rdata;
        }
    }
    ...
    this.subMethod = function (owner, exData) {
        ...
        if (typeof exPath.__rdata == "object") {
            exPath.__rdata = Mini.JsonHelper.toJson(exPath.__rdata);
        }
        ...
        var options = {
            ...
            error: function (responseText, statusText, xhr, $form) {
                ...
                console.error("链接远程服务器超时或服务器关闭.\n\n请重新登录或检查网络畅通.");
            }

        };

        if (exData.data) {
            options.data = exData.data;
        }

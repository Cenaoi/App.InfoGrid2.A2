...
namespace App.InfoGrid2.Sec.Role
{
    public partial class ModuleTree : WidgetControl, IView
    {
        ...
        private void InitDataTree()
        {
            ...
            LightModelFilter roleFilter = new LightModelFilter(typeof(SEC_ROLE_FUN));
            roleFilter.And("SEC_ROLE_ID", m_RoleID);
            roleFilter.And("VISIBLE", true);

            LModelList<SEC_ROLE_FUN> roleModules = decipher.SelectModels<SEC_ROLE_FUN>(roleFilter);
        ...
        public void Submit()
        {
            ...
            foreach (var fun in models)
            {
                ...
                SEC_ROLE_FUN roleFun = decipher.SelectToOneModel<SEC_ROLE_FUN>(roleFilter);
        ...
        private SEC_ROLE_FUN CreateModuleNode(SEC_FUN_DEF fun,bool visible,int checkStateId)
        {
            int[] arrChildIds = GetFunArrChildIds(fun.SEC_FUN_DEF_ID);

            SEC_ROLE_FUN roleFun = new SEC_ROLE_FUN();

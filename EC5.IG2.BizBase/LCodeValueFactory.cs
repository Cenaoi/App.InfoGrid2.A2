...
namespace EC5.IG2.BizBase
{
    ...
    public class LCodeValueFactory
    {
        ...
        public string[] ExecLCode(Store store, DataRecord srcRecord, LModel model)
        {
            ...
            if (model == null) { throw new ArgumentNullException("model", "实体不能为空."); }
            ...
            foreach (LcValueTable lcvTable in lcvTables)
            {
                ...
                foreach (var lcvIf in lcvTable.IfList)
                {
                    ...
                    mValue = ModelConvert.ChangeType(mValue, fieldElem);

                    bool eq = this.LogicAB(mValue, lcvValue, lcvIf.Logic, fieldElem.IsNumber);
                    ...
                    foreach (var lcvThen in lcvIf.ThenList)
                    {
                        ...
                        if (lcvThen.Logic == LcValueThenLogic.IF_NULL)
                        {
                            ...
                            {
                                string valueStr = Convert.ToString(srcValue);
                            ...
                            else if (!model.IsNull(lcvThen.Field))
                            {
                                continue;
                            }
                        }
        ...
        /// <returns></returns>
        private bool LogicAB(object a, object b, Logic logicAB, bool isNumber)
        {
            bool result = false;

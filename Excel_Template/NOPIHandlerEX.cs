...
namespace App.InfoGrid2.Excel_Template
{
    ...
    public class NOPIHandlerEX:IDisposable
    {
        ...
        public SheetPro ReadExcel(string path)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    sp.IsTotal = GetIsTotal(sheet, 6, workbook);
        ...
        /// <returns></returns>
        bool GetIsTotal(ISheet sheet, int index, IWorkbook workbook)
        {
            IRow row = sheet.GetRow(index);
        ...
        private string GetCellValue(ICell icell)
        {
            ...
            try
            {
                if (icell.CellType == CellType.Numeric)
                {
                    ...
                    if (DateUtil.IsADateFormat(i, fs))
                    {
        ...
        /// <param name="mf"></param>
        /// <returns></returns>
        public void InsertSubData(SheetPro sp, DataSet mf)
        {
            ...
            try
            {
                ...
                GetDataBaseValue(header, mf);

                ///这是处理中间数据区
                RowProCollection dateArea = sp.DataArea;
                dateArea.SpanCellAndRow.cellPro = UpdateSubSpanCell(dateArea.SpanCellAndRow.cellPro, count, num);
                ///更新模板数据
                UpdateSubData(mf, sp);
                ///把子表模板数据给移除掉
        ...
        private void GetDataBaseValue(RowProCollection item, DataSet mf) 
        {
            foreach (RowPro rp in item)
            {
        ...
        private void UpdateSubData_CellPro(LModel item, RowPro rp1,int rowIndex)
        {
            ...
            foreach (CellPro cp in rp1.cellPro)
            {
                ...
                ///这是判断是否为数字类型
                if (cp.Value.StartsWith("{$T.") && !cp.Value.Contains("-"))
                {
        ...
        private void UpdateSubData(DataSet mf,SheetPro sp) 
        {
            ...
            {
                List<RowPro> rpList = CopyData(sp.DataArea, sp.subFirstRow, sp.subLastRow);

                UpdateSubData_Model(sp.DataArea, rpList, item,++i);
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        private List<RowPro> CopyData(RowProCollection item, int subFirstRow, int subLastRow)
        {
            try
            {
                ...
                for (int i = 0; i < subLastRow - subFirstRow + 1; i++)
                {
                    RowPro rp = item[i];
        ...
        private CellProCollection UpdateSubSpanCell(CellProCollection cpSubData, int number, int num)
        {

...
namespace EasyClick.Web.Mini2
{
    ...
    public class ComboBox : TriggerBox
    {
        /// <summary>
        /// (构造函数)下拉框
        /// </summary>
        public ComboBox()
        {
            this.InReady = "Mini2.ui.form.field.ComboBox";

            this.JsNamespace = "Mini2.ui.form.field.ComboBox";
        }



        string m_ValueField = "value";

        string m_DisplayField = "text";
        ...
        private void FullScript(StringBuilder sb)
        {
            ...
            if (!StringUtil.IsBlank(this.StringItems))
            {
                ...
                foreach (string sItem in sp)
                {
                    ...
                    string keyJson = JsonUtil.ToJson(kv.Key, JsonQuotationMark.SingleQuotes);
                    string valueJson = JsonUtil.ToJson(StringUtil.NoBlank(kv.Value, kv.Key), JsonQuotationMark.SingleQuotes);

                    sb.AppendFormat("{{value:'{0}', text:'{1}'}}", keyJson, valueJson);
                }
            }

...
namespace EasyClick.Web.ReportForms
{
    ...
    public class ReportTable
    {
        ...
        /// 分组的字段格式化
        /// </summary>
        List<string> m_GroupFormats = new List<string>();

        /// <summary>
        /// 分组的字段格式化
        /// </summary>
        public List<string> GroupFormats
        {
            get { return m_GroupFormats; }
        }
        ...
        public string GetCID(LModel model, string[] groupBy)
        {
            ...
            for (int i = 0; i < groupBy.Length; i++)
            {
                ...
                format = m_GroupFormats[i];

                if (!string.IsNullOrEmpty(format))
                {
                    v = string.Format(format, model[field]);
                }
                else
                {
                    v = model[field].ToString();
                }
        ...
        private string[] GetGroupBy()
        {
            ...
            if (m_GroupFormats.Count < groups.Length)
            {
                for (int i = m_GroupFormats.Count; i < groups.Length; i++)
                {
                    m_GroupFormats.Add(null);   
                }
            }
        ...
        private void InitAuotCreateRow(string[] groupBy, LModelList<LModel> models)
        {
            ...
            foreach (LModel m in models)
            {
                ...
                for (int i = 0; i < groupBy.Length; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(format))
                    {
                        v = string.Format(format, m[field]);
                    }
                    else
                    {
                        v = m[field].ToString();
